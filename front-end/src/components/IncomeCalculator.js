...
function IncomeCalculator() {
    ...
    const handleSubmit = (e) => {
      ...
      setIncomeResults(`With an annual salary before tax of £${grossWage}, ${studentLoan} and a ${pension}% pension contribution, here's your salary breakdown:`);
      setTotalPay(calculate(grossWage, studentLoan, pension))
      // setResult("200px");
      // setSubmitOpacity("0px");

      axios.post('http://...', incomeData)
      .then(response => {
    ...
    function calculate(wage, studentLoan, pension){
      ...
      return ` £`+totalPay+` per year takehome. That's £`+(totalPay/12).toFixed(2)+` per month.`
            ...
            >
                <div className="income-inputs">
                    ...
                    <select
                        ...
                        required
                    >
                        <option value="" disabled selected> Student Loan?</option>
                        <option value="Plan 1 student loan">NI and pre-2012 Eng + Wales (Plan 1)</option>
                        <option value="Plan 2 student loan">Eng + Wales post-2012 (Plan 2)</option>
                        <option value="Plan 4 (scotland only) student loan">Scotland post-1998 (Plan 4)</option>
